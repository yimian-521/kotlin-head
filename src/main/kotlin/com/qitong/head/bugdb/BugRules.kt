...
object BugRules {
    ...
    private fun registerChunk1() {
        ...
        BugDB.load(BugRule(id="KT-0332",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Int版）",trigger="val x:Int=\\\"hi\\\"",detection="已知Int",fix="自动推断"))
        BugDB.load(BugRule(id="KT-0333",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Long版）",trigger="val x:Long=\\\"hi\\\"",detection="已知Long",fix="自动推断"))
        BugDB.load(BugRule(id="KT-0334",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Double版）",trigger="val x:Double=\\\"hi\\\"",detection="已知Double",fix="自动推断"))
        BugDB.load(BugRule(id="KT-0335",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Boolean版）",trigger="val x:Boolean=\\\"hi\\\"",detection="已知Boolean",fix="自动推断"))
        BugDB.load(BugRule(id="KT-0336",category=BugCategory.DATA_SERIAL,severity=BugSeverity.MODERATE,
            title="@SerialName与字段名不一致（Long版）",trigger="@SerialName(\\\"y\\\") val x:Long",detection="重命名混淆",fix="统一命名"))
        BugDB.load(BugRule(id="KT-0337",category=BugCategory.DATA_SERIAL,severity=BugSeverity.MODERATE,
            title="@SerialName与字段名不一致（Double版）",trigger="@SerialName(\\\"y\\\") val x:Double",detection="重命名混淆",fix="统一命名"))
        BugDB.load(BugRule(id="KT-0338",category=BugCategory.DATA_SERIAL,severity=BugSeverity.MODERATE,
    ...
    private fun registerChunk4() {
        ...
        BugDB.load(BugRule(id="KT-1784",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmStatic缺失",trigger="INSTANCE.",detection="需要INSTANCE — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0084）",fix="加@JvmStatic"))
        BugDB.load(BugRule(id="KT-1785",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1788",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@JvmField冗余",trigger="@JvmField val x=42",detection="简单常量不需要 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0088）",fix="直接const"))
        BugDB.load(BugRule(id="KT-1789",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-1790",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1791",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1793",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1806",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="@NotNull注解缺失（Boolean?）",trigger="fun javaMethod():Boolean?=null",detection="Kotlin不认Java注解 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0083）",fix="显式标注?"))
        BugDB.load(BugRule(id="KT-1807",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmStatic缺失",trigger="INSTANCE.",detection="需要INSTANCE — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0084）",fix="加@JvmStatic"))
        BugDB.load(BugRule(id="KT-1808",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="SAM转换歧义",trigger="fun f(r:Runnable);fun f(c:Callable);f{",detection="多SAM冲突 — Char类型字符编码边界,正则匹配组越界（参见 KT-0085）",fix="显式lambda类型"))
        BugDB.load(BugRule(id="KT-1809",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Kotlin集合与Java互转（Double?）",trigger="kotlinDouble?.toDouble?()在Java侧",detection="创建新副本 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0086）",fix="直接传递"))
        BugDB.load(BugRule(id="KT-1810",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmOverloads缺失（Array<Boolean>）",trigger="fun f(a:Array<Boolean>,b:Array<Boolean>=0)",detection="Java调用不提供重载 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0087）",fix="加@JvmOverloads"))
        BugDB.load(BugRule(id="KT-1811",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@JvmField冗余",trigger="@JvmField val x=42",detection="简单常量不需要 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0088）",fix="直接const"))
        BugDB.load(BugRule(id="KT-1812",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="Java异常类型在Kotlin中不可检查",trigger="try{javaMethod()}catch(e:IOException){}",detection="Kotlin不强制catch — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0202）",fix="文档标注或runCatching"))
        BugDB.load(BugRule(id="KT-1813",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmName与Kotlin名冲突",trigger="@JvmName(\\\"f\\\") fun fKotlin(){};fun f(){}",detection="字节码签名冲突 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0203）",fix="统一命名"))
        BugDB.load(BugRule(id="KT-1814",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Java通配符转Kotlin型变",trigger="Consumer<?super T>在Kotlin",detection="<in T>映射 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0204）",fix="Consumer<in T>"))
        BugDB.load(BugRule(id="KT-1815",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="companion object的JvmStatic",trigger="companion{fun create()};Java:Companion.create()",detection="需要INSTANCE — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0205）",fix="@JvmStatic"))
        BugDB.load(BugRule(id="KT-1816",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@Throws注解在Kotlin多余",trigger="@Throws(IOException::class) fun f()",detection="Kotlin不强制 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0206）",fix="仅Java交互需要"))
        ...
        BugDB.load(BugRule(id="KT-1831",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1832",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1833",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1834",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1838",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1839",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1852",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="@NotNull注解缺失（Char）",trigger="fun javaMethod():Char=null",detection="Kotlin不认Java注解 — Char类型字符编码边界,正则匹配组越界（参见 KT-0083）",fix="显式标注?"))
        BugDB.load(BugRule(id="KT-1853",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmStatic缺失",trigger="INSTANCE.",detection="需要INSTANCE — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0084）",fix="加@JvmStatic"))
        BugDB.load(BugRule(id="KT-1854",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="SAM转换歧义",trigger="fun f(r:Runnable);fun f(c:Callable);f{",detection="多SAM冲突 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0085）",fix="显式lambda类型"))
        BugDB.load(BugRule(id="KT-1855",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Kotlin集合与Java互转（Float）",trigger="kotlinFloat.toFloat()在Java侧",detection="创建新副本 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0086）",fix="直接传递"))
        BugDB.load(BugRule(id="KT-1856",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmOverloads缺失（Long?）",trigger="fun f(a:Long?,b:Long?=0)",detection="Java调用不提供重载 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0087）",fix="加@JvmOverloads"))
        BugDB.load(BugRule(id="KT-1857",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@JvmField冗余",trigger="@JvmField val x=42",detection="简单常量不需要 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0088）",fix="直接const"))
        BugDB.load(BugRule(id="KT-1858",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="Java异常类型在Kotlin中不可检查",trigger="try{javaMethod()}catch(e:IOException){}",detection="Kotlin不强制catch — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0202）",fix="文档标注或runCatching"))
        BugDB.load(BugRule(id="KT-1859",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmName与Kotlin名冲突",trigger="@JvmName(\\\"f\\\") fun fKotlin(){};fun f(){}",detection="字节码签名冲突 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0203）",fix="统一命名"))
        BugDB.load(BugRule(id="KT-1860",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Java通配符转Kotlin型变",trigger="Consumer<?super T>在Kotlin",detection="<in T>映射 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0204）",fix="Consumer<in T>"))
        BugDB.load(BugRule(id="KT-1861",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="companion object的JvmStatic",trigger="companion{fun create()};Java:Companion.create()",detection="需要INSTANCE — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0205）",fix="@JvmStatic"))
        BugDB.load(BugRule(id="KT-1862",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@Throws注解在Kotlin多余",trigger="@Throws(IOException::class) fun f()",detection="Kotlin不强制 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0206）",fix="仅Java交互需要"))
        ...
        BugDB.load(BugRule(id="KT-1875",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="@NotNull注解缺失（Double?）",trigger="fun javaMethod():Double?=null",detection="Kotlin不认Java注解 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0083）",fix="显式标注?"))
        BugDB.load(BugRule(id="KT-1876",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmStatic缺失",trigger="INSTANCE.",detection="需要INSTANCE — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0084）",fix="加@JvmStatic"))
        BugDB.load(BugRule(id="KT-1877",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="SAM转换歧义",trigger="fun f(r:Runnable);fun f(c:Callable);f{",detection="多SAM冲突 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0085）",fix="显式lambda类型"))
        BugDB.load(BugRule(id="KT-1878",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Kotlin集合与Java互转（Long?）",trigger="kotlinLong?.toLong?()在Java侧",detection="创建新副本 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0086）",fix="直接传递"))
        BugDB.load(BugRule(id="KT-1879",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmOverloads缺失（MutableList<Double>）",trigger="fun f(a:MutableMutableList<Double><Double>,b:MutableMutableList<Double><Double>=0)",detection="Java调用不提供重载 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0087）",fix="加@JvmOverloads"))
        BugDB.load(BugRule(id="KT-1880",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@JvmField冗余",trigger="@JvmField val x=42",detection="简单常量不需要 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0088）",fix="直接const"))
        BugDB.load(BugRule(id="KT-1881",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="Java异常类型在Kotlin中不可检查",trigger="try{javaMethod()}catch(e:IOException){}",detection="Kotlin不强制catch — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0202）",fix="文档标注或runCatching"))
        BugDB.load(BugRule(id="KT-1882",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmName与Kotlin名冲突",trigger="@JvmName(\\\"f\\\") fun fKotlin(){};fun f(){}",detection="字节码签名冲突 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0203）",fix="统一命名"))
        BugDB.load(BugRule(id="KT-1883",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Java通配符转Kotlin型变",trigger="Consumer<?super T>在Kotlin",detection="<in T>映射 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0204）",fix="Consumer<in T>"))
        BugDB.load(BugRule(id="KT-1884",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="companion object的JvmStatic",trigger="companion{fun create()};Java:Companion.create()",detection="需要INSTANCE — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0205）",fix="@JvmStatic"))
        BugDB.load(BugRule(id="KT-1885",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@Throws注解在Kotlin多余",trigger="@Throws(IOException::class) fun f()",detection="Kotlin不强制 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0206）",fix="仅Java交互需要"))
        ...
        BugDB.load(BugRule(id="KT-1898",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="@NotNull注解缺失（Array<Boolean>）",trigger="fun javaMethod():Array<Boolean>=null",detection="Kotlin不认Java注解 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0083）",fix="显式标注?"))
        BugDB.load(BugRule(id="KT-1899",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmStatic缺失",trigger="INSTANCE.",detection="需要INSTANCE — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0084）",fix="加@JvmStatic"))
        BugDB.load(BugRule(id="KT-1900",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="SAM转换歧义",trigger="fun f(r:Runnable);fun f(c:Callable);f{",detection="多SAM冲突 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0085）",fix="显式lambda类型"))
        BugDB.load(BugRule(id="KT-1901",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Kotlin集合与Java互转（MutableList<Double>）",trigger="kotlinMutableList<Double>.toMutableList<Double>()在Java侧",detection="创建新副本 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0086）",fix="直接传递"))
        BugDB.load(BugRule(id="KT-1902",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmOverloads缺失（Boolean）",trigger="fun f(a:Boolean,b:Boolean=0)",detection="Java调用不提供重载 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0087）",fix="加@JvmOverloads"))
        BugDB.load(BugRule(id="KT-1903",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@JvmField冗余",trigger="@JvmField val x=42",detection="简单常量不需要 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0088）",fix="直接const"))
        BugDB.load(BugRule(id="KT-1904",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.SEVERE,
            title="Java异常类型在Kotlin中不可检查",trigger="try{javaMethod()}catch(e:IOException){}",detection="Kotlin不强制catch — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0202）",fix="文档标注或runCatching"))
        BugDB.load(BugRule(id="KT-1905",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="@JvmName与Kotlin名冲突",trigger="@JvmName(\\\"f\\\") fun fKotlin(){};fun f(){}",detection="字节码签名冲突 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0203）",fix="统一命名"))
        BugDB.load(BugRule(id="KT-1906",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="Java通配符转Kotlin型变",trigger="Consumer<?super T>在Kotlin",detection="<in T>映射 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0204）",fix="Consumer<in T>"))
        BugDB.load(BugRule(id="KT-1907",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MODERATE,
            title="companion object的JvmStatic",trigger="companion{fun create()};Java:Companion.create()",detection="需要INSTANCE — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0205）",fix="@JvmStatic"))
        BugDB.load(BugRule(id="KT-1908",category=BugCategory.JAVA_INTEROP,severity=BugSeverity.MILD,
            title="@Throws注解在Kotlin多余",trigger="@Throws(IOException::class) fun f()",detection="Kotlin不强制 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0206）",fix="仅Java交互需要"))
        BugDB.load(BugRule(id="KT-1909",category=BugCategory.SMART_CAST,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-1910",category=BugCategory.SMART_CAST,severity=BugSeverity.SEVERE,
            title="when穷举缺失",trigger="when(sealed){is A->...}",detection="遗漏子类 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0090）",fix="加else分支"))
        BugDB.load(BugRule(id="KT-1911",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1912",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1913",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
            title="is检查后类型窄化丢失（Char）",trigger="if(x is Char<*>){x[0]}",detection="擦除后泛型信息丢失 — Char类型字符编码边界,正则匹配组越界（参见 KT-0093）",fix="reified"))
        BugDB.load(BugRule(id="KT-1914",category=BugCategory.SMART_CAST,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1915",category=BugCategory.SMART_CAST,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-1916",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1917",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1918",category=BugCategory.SMART_CAST,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1948",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1949",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1950",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1951",category=BugCategory.SMART_CAST,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1952",category=BugCategory.SMART_CAST,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-1953",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1954",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1955",category=BugCategory.SMART_CAST,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1985",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1986",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1987",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1988",category=BugCategory.SMART_CAST,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-1989",category=BugCategory.SMART_CAST,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-1990",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1991",category=BugCategory.SMART_CAST,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-1992",category=BugCategory.SMART_CAST,severity=BugSeverity.MILD,
    ...
    private fun registerChunk5() {
        ...
        BugDB.load(BugRule(id="KT-2022",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2023",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2024",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2025",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2026",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2043",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
            title="内联类实现接口（Any?）",trigger="value class N(val s:Any?):Iface",detection="间接装箱 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0096）",fix="避免接口"))
        BugDB.load(BugRule(id="KT-2044",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MILD,
            title="内联类多属性（String）",trigger="@JvmInline value class P(val x:String,val y:String)",detection="仅单属性 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0097）",fix="拆成两个"))
        BugDB.load(BugRule(id="KT-2045",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
            title="内联类在泛型中装箱（Byte）",trigger="val x:Byte<MyValueClass>=listOf(MyValueClass(1))",detection="放入泛型装箱 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0213）",fix="考虑直接使用List<Int>"))
        BugDB.load(BugRule(id="KT-2046",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
            title="内联类==比较失效（Boolean?）",trigger="inline class N(val i:Boolean?);N(1)==N(1)",detection="equals自动生成 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0214）",fix="可用但注意引用比较"))
        ...
        BugDB.load(BugRule(id="KT-2063",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
            title="内联类实现接口（Int）",trigger="value class N(val s:Int):Iface",detection="间接装箱 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0096）",fix="避免接口"))
        BugDB.load(BugRule(id="KT-2064",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MILD,
            title="内联类多属性（Short）",trigger="@JvmInline value class P(val x:Short,val y:Short)",detection="仅单属性 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0097）",fix="拆成两个"))
        BugDB.load(BugRule(id="KT-2065",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
            title="内联类在泛型中装箱（Any?）",trigger="val x:Any?<MyValueClass>=listOf(MyValueClass(1))",detection="放入泛型装箱 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0213）",fix="考虑直接使用List<Int>"))
        BugDB.load(BugRule(id="KT-2066",category=BugCategory.VALUE_CLASS,severity=BugSeverity.MODERATE,
            title="内联类==比较失效（String）",trigger="inline class N(val i:String);N(1)==N(1)",detection="equals自动生成 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0214）",fix="可用但注意引用比较"))
        ...
        BugDB.load(BugRule(id="KT-2084",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2085",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2086",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2087",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举name属性依赖",trigger="when(e.name){\\\"A\\\"->1}",detection="name不是业务标识 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0217）",fix="用自定义属性"))
        BugDB.load(BugRule(id="KT-2088",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2093",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="sealed class跨文件",trigger="sealed class A;File1:class B:A();File2:class C:A()",detection="限制同文件 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0099）",fix="移到同文件或sealed interface"))
        BugDB.load(BugRule(id="KT-2094",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2095",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举包含可变状态（Double）",trigger="enum class E(var x:Double)",detection="枚举应不可变 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0101）",fix="val"))
        BugDB.load(BugRule(id="KT-2096",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="when穷举sealed少子类",trigger="sealed class A;class B:A();when(a){is B->1}",detection="新增C:A()不报错 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0216）",fix="加else或sealed interface"))
        BugDB.load(BugRule(id="KT-2097",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举name属性依赖",trigger="when(e.name){\\\"A\\\"->1}",detection="name不是业务标识 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0217）",fix="用自定义属性"))
        BugDB.load(BugRule(id="KT-2098",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举构造函数开销（Long）",trigger="enum class E(val x:Long=0);E.A",detection="每个枚举实例化 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0218）",fix="用object代替"))
        ...
        BugDB.load(BugRule(id="KT-2103",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="sealed class跨文件",trigger="sealed class A;File1:class B:A();File2:class C:A()",detection="限制同文件 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0099）",fix="移到同文件或sealed interface"))
        BugDB.load(BugRule(id="KT-2104",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2105",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举包含可变状态（Byte）",trigger="enum class E(var x:Byte)",detection="枚举应不可变 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0101）",fix="val"))
        BugDB.load(BugRule(id="KT-2106",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="when穷举sealed少子类",trigger="sealed class A;class B:A();when(a){is B->1}",detection="新增C:A()不报错 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0216）",fix="加else或sealed interface"))
        BugDB.load(BugRule(id="KT-2107",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举name属性依赖",trigger="when(e.name){\\\"A\\\"->1}",detection="name不是业务标识 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0217）",fix="用自定义属性"))
        BugDB.load(BugRule(id="KT-2108",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举构造函数开销（Char）",trigger="enum class E(val x:Char=0);E.A",detection="每个枚举实例化 — Char类型字符编码边界,正则匹配组越界（参见 KT-0218）",fix="用object代替"))
        ...
        BugDB.load(BugRule(id="KT-2114",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2116",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2118",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2123",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="sealed class跨文件",trigger="sealed class A;File1:class B:A();File2:class C:A()",detection="限制同文件 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0099）",fix="移到同文件或sealed interface"))
        BugDB.load(BugRule(id="KT-2124",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2125",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举包含可变状态（Any?）",trigger="enum class E(var x:Any?)",detection="枚举应不可变 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0101）",fix="val"))
        BugDB.load(BugRule(id="KT-2126",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="when穷举sealed少子类",trigger="sealed class A;class B:A();when(a){is B->1}",detection="新增C:A()不报错 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0216）",fix="加else或sealed interface"))
        BugDB.load(BugRule(id="KT-2127",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举name属性依赖",trigger="when(e.name){\\\"A\\\"->1}",detection="name不是业务标识 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0217）",fix="用自定义属性"))
        BugDB.load(BugRule(id="KT-2128",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举构造函数开销（Boolean?）",trigger="enum class E(val x:Boolean?=0);E.A",detection="每个枚举实例化 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0218）",fix="用object代替"))
        ...
        BugDB.load(BugRule(id="KT-2133",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="sealed class跨文件",trigger="sealed class A;File1:class B:A();File2:class C:A()",detection="限制同文件 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0099）",fix="移到同文件或sealed interface"))
        BugDB.load(BugRule(id="KT-2134",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2135",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举包含可变状态（MutableList<Double>）",trigger="enum class E(var x:MutableMutableList<Double><Double>)",detection="枚举应不可变 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0101）",fix="val"))
        BugDB.load(BugRule(id="KT-2136",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="when穷举sealed少子类",trigger="sealed class A;class B:A();when(a){is B->1}",detection="新增C:A()不报错 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0216）",fix="加else或sealed interface"))
        BugDB.load(BugRule(id="KT-2137",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举name属性依赖",trigger="when(e.name){\\\"A\\\"->1}",detection="name不是业务标识 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0217）",fix="用自定义属性"))
        BugDB.load(BugRule(id="KT-2138",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举构造函数开销（Map<String,Int>）",trigger="enum class E(val x:Map<String,Int>=0);E.A",detection="每个枚举实例化 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0218）",fix="用object代替"))
        ...
        BugDB.load(BugRule(id="KT-2143",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="sealed class跨文件",trigger="sealed class A;File1:class B:A();File2:class C:A()",detection="限制同文件 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0099）",fix="移到同文件或sealed interface"))
        BugDB.load(BugRule(id="KT-2144",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2145",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举包含可变状态",trigger="enum class E(var x:Int)",detection="枚举应不可变 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0101）",fix="val"))
        BugDB.load(BugRule(id="KT-2146",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="when穷举sealed少子类",trigger="sealed class A;class B:A();when(a){is B->1}",detection="新增C:A()不报错 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0216）",fix="加else或sealed interface"))
        BugDB.load(BugRule(id="KT-2147",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举name属性依赖",trigger="when(e.name){\\\"A\\\"->1}",detection="name不是业务标识 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0217）",fix="用自定义属性"))
        BugDB.load(BugRule(id="KT-2148",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举构造函数开销（String）",trigger="enum class E(val x:String=0);E.A",detection="每个枚举实例化 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0218）",fix="用object代替"))
        ...
        BugDB.load(BugRule(id="KT-2154",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2155",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2163",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="sealed class跨文件",trigger="sealed class A;File1:class B:A();File2:class C:A()",detection="限制同文件 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0099）",fix="移到同文件或sealed interface"))
        BugDB.load(BugRule(id="KT-2164",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举ordinal依赖",trigger="when(e.ordinal){0->...}",detection="顺序变化会错 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0100）",fix="用枚举常量"))
        BugDB.load(BugRule(id="KT-2165",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举包含可变状态（Any）",trigger="enum class E(var x:Any)",detection="枚举应不可变 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0101）",fix="val"))
        BugDB.load(BugRule(id="KT-2166",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="when穷举sealed少子类",trigger="sealed class A;class B:A();when(a){is B->1}",detection="新增C:A()不报错 — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0216）",fix="加else或sealed interface"))
        BugDB.load(BugRule(id="KT-2167",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MODERATE,
            title="枚举name属性依赖",trigger="when(e.name){\\\"A\\\"->1}",detection="name不是业务标识 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0217）",fix="用自定义属性"))
        BugDB.load(BugRule(id="KT-2168",category=BugCategory.SEALED_ENUM,severity=BugSeverity.MILD,
            title="枚举构造函数开销（Short）",trigger="enum class E(val x:Short=0);E.A",detection="每个枚举实例化 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0218）",fix="用object代替"))
        ...
        BugDB.load(BugRule(id="KT-2174",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2175",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2176",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2177",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2179",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2181",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2182",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2184",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2186",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2188",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2189",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2191",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2193",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2195",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2196",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2198",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2200",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2202",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2203",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2205",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2207",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2209",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2210",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2212",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2214",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2216",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2217",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2219",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2221",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2223",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2224",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2226",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2228",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2230",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2231",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2233",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2235",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2237",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2238",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2240",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2242",category=BugCategory.DELEGATE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2244",category=BugCategory.DELEGATE,severity=BugSeverity.MILD,
            title="不必要的委托",trigger="val x by lazy{42}",detection="常量不需要lazy — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0104）",fix="直接val=42"))
        BugDB.load(BugRule(id="KT-2245",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2246",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2247",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2248",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2249",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Thread.sleep在协程",trigger="suspend fun f(){Thread.sleep(1000)}",detection="阻塞线程 — Char类型字符编码边界,正则匹配组越界（参见 KT-0109）",fix="delay"))
        BugDB.load(BugRule(id="KT-2250",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2251",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2253",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2256",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2257",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2258",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2259",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2260",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2262",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2263",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2264",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2267",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2269",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2277",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺陷",trigger="if(x==null){synchronized(this){if(x==null){x=f()}}}",detection="DCL需要volatile — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0106）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2278",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="Synchronized内阻塞操作",trigger="synchronized(lock){doSlowIO()}",detection="持有锁时阻塞 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0107）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2279",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="死锁风险",trigger="fun a(){lock1;lock2} fun b(){lock2;lock1}",detection="反向获取锁 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0108）",fix="统一锁顺序"))
        BugDB.load(BugRule(id="KT-2280",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Thread.sleep在协程",trigger="suspend fun f(){Thread.sleep(1000)}",detection="阻塞线程 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0109）",fix="delay"))
        BugDB.load(BugRule(id="KT-2281",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="volatile缺失",trigger="var flag=false;thread{flag=true}",detection="不可见 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0110）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2282",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="不必要的同步",trigger="synchronized(val x=42){}",detection="简单赋值不需要 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0111）",fix="直接赋值"))
        BugDB.load(BugRule(id="KT-2283",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="不可见赋值+volatile缺失",trigger="var flag=false;thread{flag=true};while(!flag){}",detection="不可见导致死循环 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0222）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2284",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺少局部变量",trigger="if(x==null){synchronized{val t=f();if(x==null)x=t}}",detection="DCL标准写法 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0223）",fix="局部val=instance;if(...)"))
        BugDB.load(BugRule(id="KT-2285",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="AtomicReference的ABA问题",trigger="val ref=AtomicReference(0);ref.compareAndSet(0,1);...;ref.compareAndSet(1,0)",detection="ABA — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0224）",fix="AtomicStampedReference"))
        BugDB.load(BugRule(id="KT-2286",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="并发HashMap的putIfAbsent竞态",trigger="map.putIfAbsent(k,calc());map[k]",detection="calc先执行 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0225）",fix="computeIfAbsent(k){calc()}"))
        BugDB.load(BugRule(id="KT-2287",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="锁粒度太粗",trigger="synchronized(this){a();b();c();sleep()}",detection="阻塞所有 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0226）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2288",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Condition未在锁内使用",trigger="lock.newCondition().await()",detection="必须在锁内 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0227）",fix="lock.withLock{cond.await()}"))
        BugDB.load(BugRule(id="KT-2289",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="并发集合不必要（String）",trigger="Collections.synchronizedString(arrayString)",detection="一次初始化 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0228）",fix="直接ArrayList"))
        BugDB.load(BugRule(id="KT-2290",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="父进程独裁导致子进程全灭",trigger="ProcessCoordinator.setStyle(DICTATOR)",detection="父进程治理风格过激 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0256）",fix="FEDERAL或CONTRACT"))
        BugDB.load(BugRule(id="KT-2291",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="子进程杀死父进程",trigger="launch{parentJob.cancel()}",detection="结构化并发中子取消父 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0257）",fix="子进程不应持有parentJob引用"))
        BugDB.load(BugRule(id="KT-2292",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="指挥官互相争夺同一子进程",trigger="两个@Commander注解同一tag",detection="重复声明 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0258）",fix="一个tag一个指挥官"))
        BugDB.load(BugRule(id="KT-2293",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="进程树中孙子进程孤立",trigger="launch{launch{launch{}};cancel中间层",detection="中间层取消后孙子悬空 — Char类型字符编码边界,正则匹配组越界（参见 KT-0259）",fix="supervisorScope+Job树检查"))
        BugDB.load(BugRule(id="KT-2294",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="子进程反过来指挥父进程",trigger="@ProcessBody中调用ProcessCoordinator.setStyle()",detection="子进程越权修改全局策略 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0285）",fix="子进程只读不写"))
        BugDB.load(BugRule(id="KT-2295",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="指挥官被进程体反杀",trigger="@Commander中调用自己tag的@ProcessBody杀自己",detection="自噬 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0286）",fix="Commander不参与ProcessBody"))
        BugDB.load(BugRule(id="KT-2296",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="检测进程报告一切正常但全线崩溃",trigger="watchTag返回healthy=true但summary全是✖",detection="哨兵谎报军情 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0287）",fix="cross-check其他哨兵"))
        BugDB.load(BugRule(id="KT-2297",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="依赖图标记已解决实际未解决",trigger="DependencyGraph标记conflict_resolved但detectConflicts仍有",detection="假解决 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0288）",fix="解析后立即re-check"))
        BugDB.load(BugRule(id="KT-2298",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="EventBus取消订阅后仍收到事件",trigger="unsubscribe(tag);emit(tag,ev);仍收到",detection="取消订阅延迟 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0291）",fix="unsubscribe后立即yield"))
        BugDB.load(BugRule(id="KT-2299",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="synchronized+协程=阻塞线程池",trigger="synchronized(lock){delay(1000)}",detection="synchronized内不可suspend但delay会挂 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0304）",fix="Mutex.withLock+delay"))
        BugDB.load(BugRule(id="KT-2300",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="单线程协程拿到了并行结果，多线程反而串行",trigger="coroutineScope{async{a};async{b};awaitAll}",detection="协程调度并行，线程锁串行 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0310）",fix="多线程用无锁数据结构"))
        BugDB.load(BugRule(id="KT-2301",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="单线程expect但实际是async语义",trigger="val x=async{a};val y=async{b};x.await()+y.await()",detection="async启动即执行不是lazy — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0311）",fix="如需lazy用CoroutineStart.LAZY"))
        BugDB.load(BugRule(id="KT-2302",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Dispatchers.Default单核跑出并行幻觉",trigger="单核CPU上launch(Default){a;b;c}",detection="Default线程池>1核=真并行但单核=并发 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0312）",fix="明确用newSingleThreadContext"))
        BugDB.load(BugRule(id="KT-2303",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="多线程加锁反而比单线程更慢",trigger="1000线程竞争同一把synchronized锁",detection="锁竞争+上下文切换>计算本身 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0313）",fix="缩小临界区或用无锁CAS"))
        ...
        BugDB.load(BugRule(id="KT-2309",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="Synchronized内阻塞操作",trigger="synchronized(lock){doSlowIO()}",detection="持有锁时阻塞 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0107）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2310",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2312",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2313",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2316",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2324",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2325",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2327",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2328",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2329",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2330",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2334",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2339",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺陷",trigger="if(x==null){synchronized(this){if(x==null){x=f()}}}",detection="DCL需要volatile — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0106）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2340",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="Synchronized内阻塞操作",trigger="synchronized(lock){doSlowIO()}",detection="持有锁时阻塞 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0107）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2341",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="死锁风险",trigger="fun a(){lock1;lock2} fun b(){lock2;lock1}",detection="反向获取锁 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0108）",fix="统一锁顺序"))
        BugDB.load(BugRule(id="KT-2342",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Thread.sleep在协程",trigger="suspend fun f(){Thread.sleep(1000)}",detection="阻塞线程 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0109）",fix="delay"))
        BugDB.load(BugRule(id="KT-2343",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="volatile缺失",trigger="var flag=false;thread{flag=true}",detection="不可见 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0110）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2344",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="不必要的同步",trigger="synchronized(val x=42){}",detection="简单赋值不需要 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0111）",fix="直接赋值"))
        BugDB.load(BugRule(id="KT-2345",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="不可见赋值+volatile缺失",trigger="var flag=false;thread{flag=true};while(!flag){}",detection="不可见导致死循环 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0222）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2346",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺少局部变量",trigger="if(x==null){synchronized{val t=f();if(x==null)x=t}}",detection="DCL标准写法 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0223）",fix="局部val=instance;if(...)"))
        BugDB.load(BugRule(id="KT-2347",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="AtomicReference的ABA问题",trigger="val ref=AtomicReference(0);ref.compareAndSet(0,1);...;ref.compareAndSet(1,0)",detection="ABA — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0224）",fix="AtomicStampedReference"))
        BugDB.load(BugRule(id="KT-2348",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="并发HashMap的putIfAbsent竞态",trigger="map.putIfAbsent(k,calc());map[k]",detection="calc先执行 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0225）",fix="computeIfAbsent(k){calc()}"))
        BugDB.load(BugRule(id="KT-2349",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="锁粒度太粗",trigger="synchronized(this){a();b();c();sleep()}",detection="阻塞所有 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0226）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2350",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Condition未在锁内使用",trigger="lock.newCondition().await()",detection="必须在锁内 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0227）",fix="lock.withLock{cond.await()}"))
        BugDB.load(BugRule(id="KT-2351",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="并发集合不必要（List<String>）",trigger="Collections.synchronizedList<String>(arrayList<String>)",detection="一次初始化 — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0228）",fix="直接ArrayList"))
        BugDB.load(BugRule(id="KT-2352",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="父进程独裁导致子进程全灭",trigger="ProcessCoordinator.setStyle(DICTATOR)",detection="父进程治理风格过激 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0256）",fix="FEDERAL或CONTRACT"))
        BugDB.load(BugRule(id="KT-2353",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="子进程杀死父进程",trigger="launch{parentJob.cancel()}",detection="结构化并发中子取消父 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0257）",fix="子进程不应持有parentJob引用"))
        BugDB.load(BugRule(id="KT-2354",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="指挥官互相争夺同一子进程",trigger="两个@Commander注解同一tag",detection="重复声明 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0258）",fix="一个tag一个指挥官"))
        BugDB.load(BugRule(id="KT-2355",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="进程树中孙子进程孤立",trigger="launch{launch{launch{}};cancel中间层",detection="中间层取消后孙子悬空 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0259）",fix="supervisorScope+Job树检查"))
        BugDB.load(BugRule(id="KT-2356",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="子进程反过来指挥父进程",trigger="@ProcessBody中调用ProcessCoordinator.setStyle()",detection="子进程越权修改全局策略 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0285）",fix="子进程只读不写"))
        BugDB.load(BugRule(id="KT-2357",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="指挥官被进程体反杀",trigger="@Commander中调用自己tag的@ProcessBody杀自己",detection="自噬 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0286）",fix="Commander不参与ProcessBody"))
        BugDB.load(BugRule(id="KT-2358",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="检测进程报告一切正常但全线崩溃",trigger="watchTag返回healthy=true但summary全是✖",detection="哨兵谎报军情 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0287）",fix="cross-check其他哨兵"))
        BugDB.load(BugRule(id="KT-2359",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="依赖图标记已解决实际未解决",trigger="DependencyGraph标记conflict_resolved但detectConflicts仍有",detection="假解决 — Char类型字符编码边界,正则匹配组越界（参见 KT-0288）",fix="解析后立即re-check"))
        BugDB.load(BugRule(id="KT-2360",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="EventBus取消订阅后仍收到事件",trigger="unsubscribe(tag);emit(tag,ev);仍收到",detection="取消订阅延迟 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0291）",fix="unsubscribe后立即yield"))
        BugDB.load(BugRule(id="KT-2361",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="synchronized+协程=阻塞线程池",trigger="synchronized(lock){delay(1000)}",detection="synchronized内不可suspend但delay会挂 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0304）",fix="Mutex.withLock+delay"))
        BugDB.load(BugRule(id="KT-2362",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="单线程协程拿到了并行结果，多线程反而串行",trigger="coroutineScope{async{a};async{b};awaitAll}",detection="协程调度并行，线程锁串行 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0310）",fix="多线程用无锁数据结构"))
        BugDB.load(BugRule(id="KT-2363",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="单线程expect但实际是async语义",trigger="val x=async{a};val y=async{b};x.await()+y.await()",detection="async启动即执行不是lazy — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0311）",fix="如需lazy用CoroutineStart.LAZY"))
        BugDB.load(BugRule(id="KT-2364",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Dispatchers.Default单核跑出并行幻觉",trigger="单核CPU上launch(Default){a;b;c}",detection="Default线程池>1核=真并行但单核=并发 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0312）",fix="明确用newSingleThreadContext"))
        BugDB.load(BugRule(id="KT-2365",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="多线程加锁反而比单线程更慢",trigger="1000线程竞争同一把synchronized锁",detection="锁竞争+上下文切换>计算本身 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0313）",fix="缩小临界区或用无锁CAS"))
        ...
        BugDB.load(BugRule(id="KT-2370",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺陷",trigger="if(x==null){synchronized(this){if(x==null){x=f()}}}",detection="DCL需要volatile — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0106）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2371",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="Synchronized内阻塞操作",trigger="synchronized(lock){doSlowIO()}",detection="持有锁时阻塞 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0107）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2372",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="死锁风险",trigger="fun a(){lock1;lock2} fun b(){lock2;lock1}",detection="反向获取锁 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0108）",fix="统一锁顺序"))
        BugDB.load(BugRule(id="KT-2373",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Thread.sleep在协程",trigger="suspend fun f(){Thread.sleep(1000)}",detection="阻塞线程 — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0109）",fix="delay"))
        BugDB.load(BugRule(id="KT-2374",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="volatile缺失",trigger="var flag=false;thread{flag=true}",detection="不可见 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0110）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2375",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="不必要的同步",trigger="synchronized(val x=42){}",detection="简单赋值不需要 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0111）",fix="直接赋值"))
        BugDB.load(BugRule(id="KT-2376",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="不可见赋值+volatile缺失",trigger="var flag=false;thread{flag=true};while(!flag){}",detection="不可见导致死循环 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0222）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2377",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺少局部变量",trigger="if(x==null){synchronized{val t=f();if(x==null)x=t}}",detection="DCL标准写法 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0223）",fix="局部val=instance;if(...)"))
        BugDB.load(BugRule(id="KT-2378",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="AtomicReference的ABA问题",trigger="val ref=AtomicReference(0);ref.compareAndSet(0,1);...;ref.compareAndSet(1,0)",detection="ABA — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0224）",fix="AtomicStampedReference"))
        BugDB.load(BugRule(id="KT-2379",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="并发HashMap的putIfAbsent竞态",trigger="map.putIfAbsent(k,calc());map[k]",detection="calc先执行 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0225）",fix="computeIfAbsent(k){calc()}"))
        BugDB.load(BugRule(id="KT-2380",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="锁粒度太粗",trigger="synchronized(this){a();b();c();sleep()}",detection="阻塞所有 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0226）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2381",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Condition未在锁内使用",trigger="lock.newCondition().await()",detection="必须在锁内 — Char类型字符编码边界,正则匹配组越界（参见 KT-0227）",fix="lock.withLock{cond.await()}"))
        BugDB.load(BugRule(id="KT-2382",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="并发集合不必要（Double?）",trigger="Collections.synchronizedDouble?(arrayDouble?)",detection="一次初始化 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0228）",fix="直接ArrayList"))
        BugDB.load(BugRule(id="KT-2383",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="父进程独裁导致子进程全灭",trigger="ProcessCoordinator.setStyle(DICTATOR)",detection="父进程治理风格过激 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0256）",fix="FEDERAL或CONTRACT"))
        BugDB.load(BugRule(id="KT-2384",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="子进程杀死父进程",trigger="launch{parentJob.cancel()}",detection="结构化并发中子取消父 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0257）",fix="子进程不应持有parentJob引用"))
        BugDB.load(BugRule(id="KT-2385",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="指挥官互相争夺同一子进程",trigger="两个@Commander注解同一tag",detection="重复声明 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0258）",fix="一个tag一个指挥官"))
        BugDB.load(BugRule(id="KT-2386",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="进程树中孙子进程孤立",trigger="launch{launch{launch{}};cancel中间层",detection="中间层取消后孙子悬空 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0259）",fix="supervisorScope+Job树检查"))
        BugDB.load(BugRule(id="KT-2387",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="子进程反过来指挥父进程",trigger="@ProcessBody中调用ProcessCoordinator.setStyle()",detection="子进程越权修改全局策略 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0285）",fix="子进程只读不写"))
        BugDB.load(BugRule(id="KT-2388",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="指挥官被进程体反杀",trigger="@Commander中调用自己tag的@ProcessBody杀自己",detection="自噬 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0286）",fix="Commander不参与ProcessBody"))
        BugDB.load(BugRule(id="KT-2389",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="检测进程报告一切正常但全线崩溃",trigger="watchTag返回healthy=true但summary全是✖",detection="哨兵谎报军情 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0287）",fix="cross-check其他哨兵"))
        BugDB.load(BugRule(id="KT-2390",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="依赖图标记已解决实际未解决",trigger="DependencyGraph标记conflict_resolved但detectConflicts仍有",detection="假解决 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0288）",fix="解析后立即re-check"))
        BugDB.load(BugRule(id="KT-2391",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="EventBus取消订阅后仍收到事件",trigger="unsubscribe(tag);emit(tag,ev);仍收到",detection="取消订阅延迟 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0291）",fix="unsubscribe后立即yield"))
        BugDB.load(BugRule(id="KT-2392",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="synchronized+协程=阻塞线程池",trigger="synchronized(lock){delay(1000)}",detection="synchronized内不可suspend但delay会挂 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0304）",fix="Mutex.withLock+delay"))
        BugDB.load(BugRule(id="KT-2393",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="单线程协程拿到了并行结果，多线程反而串行",trigger="coroutineScope{async{a};async{b};awaitAll}",detection="协程调度并行，线程锁串行 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0310）",fix="多线程用无锁数据结构"))
        BugDB.load(BugRule(id="KT-2394",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="单线程expect但实际是async语义",trigger="val x=async{a};val y=async{b};x.await()+y.await()",detection="async启动即执行不是lazy — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0311）",fix="如需lazy用CoroutineStart.LAZY"))
        BugDB.load(BugRule(id="KT-2395",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Dispatchers.Default单核跑出并行幻觉",trigger="单核CPU上launch(Default){a;b;c}",detection="Default线程池>1核=真并行但单核=并发 — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0312）",fix="明确用newSingleThreadContext"))
        BugDB.load(BugRule(id="KT-2396",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="多线程加锁反而比单线程更慢",trigger="1000线程竞争同一把synchronized锁",detection="锁竞争+上下文切换>计算本身 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0313）",fix="缩小临界区或用无锁CAS"))
        ...
        BugDB.load(BugRule(id="KT-2401",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺陷",trigger="if(x==null){synchronized(this){if(x==null){x=f()}}}",detection="DCL需要volatile — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0106）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2402",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="Synchronized内阻塞操作",trigger="synchronized(lock){doSlowIO()}",detection="持有锁时阻塞 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0107）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2403",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="死锁风险",trigger="fun a(){lock1;lock2} fun b(){lock2;lock1}",detection="反向获取锁 — Char类型字符编码边界,正则匹配组越界（参见 KT-0108）",fix="统一锁顺序"))
        BugDB.load(BugRule(id="KT-2404",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Thread.sleep在协程",trigger="suspend fun f(){Thread.sleep(1000)}",detection="阻塞线程 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0109）",fix="delay"))
        BugDB.load(BugRule(id="KT-2405",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="volatile缺失",trigger="var flag=false;thread{flag=true}",detection="不可见 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0110）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2406",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="不必要的同步",trigger="synchronized(val x=42){}",detection="简单赋值不需要 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0111）",fix="直接赋值"))
        BugDB.load(BugRule(id="KT-2407",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="不可见赋值+volatile缺失",trigger="var flag=false;thread{flag=true};while(!flag){}",detection="不可见导致死循环 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0222）",fix="@Volatile"))
        BugDB.load(BugRule(id="KT-2408",category=BugCategory.CONCURRENCY,severity=BugSeverity.SEVERE,
            title="双重检查锁定缺少局部变量",trigger="if(x==null){synchronized{val t=f();if(x==null)x=t}}",detection="DCL标准写法 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0223）",fix="局部val=instance;if(...)"))
        BugDB.load(BugRule(id="KT-2409",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="AtomicReference的ABA问题",trigger="val ref=AtomicReference(0);ref.compareAndSet(0,1);...;ref.compareAndSet(1,0)",detection="ABA — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0224）",fix="AtomicStampedReference"))
        BugDB.load(BugRule(id="KT-2410",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="并发HashMap的putIfAbsent竞态",trigger="map.putIfAbsent(k,calc());map[k]",detection="calc先执行 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0225）",fix="computeIfAbsent(k){calc()}"))
        BugDB.load(BugRule(id="KT-2411",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="锁粒度太粗",trigger="synchronized(this){a();b();c();sleep()}",detection="阻塞所有 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0226）",fix="缩小同步块"))
        BugDB.load(BugRule(id="KT-2412",category=BugCategory.CONCURRENCY,severity=BugSeverity.MODERATE,
            title="Condition未在锁内使用",trigger="lock.newCondition().await()",detection="必须在锁内 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0227）",fix="lock.withLock{cond.await()}"))
        BugDB.load(BugRule(id="KT-2413",category=BugCategory.CONCURRENCY,severity=BugSeverity.MILD,
            title="并发集合不必要（String?）",trigger="Collections.synchronizedString?(arrayString?)",detection="一次初始化 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0228）",fix="直接ArrayList"))
        BugDB.load(BugRule(id="KT-2414",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2415",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
            title="重组副作用",trigger="LaunchedEffect(Unit){load()}",detection="key=Unit只执行一次 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0113）",fix="正确的key"))
        BugDB.load(BugRule(id="KT-2416",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2417",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2418",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
            title="不必要的remember",trigger="remember{\\\"static\\\"}",detection="常量不需要remember — Char类型字符编码边界,正则匹配组越界（参见 KT-0116）",fix="直接使用"))
        BugDB.load(BugRule(id="KT-2419",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2422",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2423",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2430",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
            title="状态提升缺失",trigger="@Composable fun C(){val s=remember{;CF(s)}",detection="子组件不可控 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0115）",fix="状态提升"))
        BugDB.load(BugRule(id="KT-2431",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
            title="不必要的remember",trigger="remember{\\\"static\\\"}",detection="常量不需要remember — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0116）",fix="直接使用"))
        BugDB.load(BugRule(id="KT-2432",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2435",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2436",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2443",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2444",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
            title="不必要的remember",trigger="remember{\\\"static\\\"}",detection="常量不需要remember — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0116）",fix="直接使用"))
        BugDB.load(BugRule(id="KT-2445",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2448",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2449",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2456",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
            title="状态提升缺失",trigger="@Composable fun C(){val s=remember{;CF(s)}",detection="子组件不可控 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0115）",fix="状态提升"))
        BugDB.load(BugRule(id="KT-2457",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
            title="不必要的remember",trigger="remember{\\\"static\\\"}",detection="常量不需要remember — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0116）",fix="直接使用"))
        BugDB.load(BugRule(id="KT-2458",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2461",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2462",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2469",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2470",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2471",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2474",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2475",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2482",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
            title="状态提升缺失",trigger="@Composable fun C(){val s=remember{;CF(s)}",detection="子组件不可控 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0115）",fix="状态提升"))
        BugDB.load(BugRule(id="KT-2483",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2484",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2487",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2488",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2495",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2496",category=BugCategory.COMPOSE,severity=BugSeverity.MILD,
            title="不必要的remember",trigger="remember{\\\"static\\\"}",detection="常量不需要remember — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0116）",fix="直接使用"))
        BugDB.load(BugRule(id="KT-2497",category=BugCategory.COMPOSE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2500",category=BugCategory.COMPOSE,severity=BugSeverity.MODERATE,
    ...
    private fun registerChunk6() {
        BugDB.load(BugRule(id="KT-2501",category=BugCategory.KMP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2502",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2503",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="iOS与JVM路径分隔符",trigger="File(\\\"a/b\\\")在iOS",detection="平台路径差异 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0119）",fix="expect/actual或Path"))
        BugDB.load(BugRule(id="KT-2504",category=BugCategory.KMP,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2505",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2521",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="iOS与JVM路径分隔符",trigger="File(\\\"a/b\\\")在iOS",detection="平台路径差异 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0119）",fix="expect/actual或Path"))
        BugDB.load(BugRule(id="KT-2522",category=BugCategory.KMP,severity=BugSeverity.MILD,
            title="commonMain中使用java.*",trigger="import java.io.File",detection="不可移植 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0120）",fix="expect class"))
        BugDB.load(BugRule(id="KT-2523",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="expect声明缺少actual",trigger="expect fun format(d:Double):String",detection="iOS无actual — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0234）",fix="补actual"))
        BugDB.load(BugRule(id="KT-2524",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="commonTest中使用平台API",trigger="assertEquals(System.currentTimeMillis(),ts)",detection="平台特定 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0235）",fix="expect/actual测试"))
        ...
        BugDB.load(BugRule(id="KT-2539",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="iOS与JVM路径分隔符",trigger="File(\\\"a/b\\\")在iOS",detection="平台路径差异 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0119）",fix="expect/actual或Path"))
        BugDB.load(BugRule(id="KT-2540",category=BugCategory.KMP,severity=BugSeverity.MILD,
            title="commonMain中使用java.*",trigger="import java.io.File",detection="不可移植 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0120）",fix="expect class"))
        BugDB.load(BugRule(id="KT-2541",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2542",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2556",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="平台API未抽象",trigger="fun a(){System.loadLibrary(\\\"x\\\")}",detection="仅JVM可用 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0118）",fix="expect/actual封装"))
        BugDB.load(BugRule(id="KT-2557",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="iOS与JVM路径分隔符",trigger="File(\\\"a/b\\\")在iOS",detection="平台路径差异 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0119）",fix="expect/actual或Path"))
        BugDB.load(BugRule(id="KT-2558",category=BugCategory.KMP,severity=BugSeverity.MILD,
            title="commonMain中使用java.*",trigger="import java.io.File",detection="不可移植 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0120）",fix="expect class"))
        BugDB.load(BugRule(id="KT-2559",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="expect声明缺少actual（String?）",trigger="expect fun format(d:Double):String?",detection="iOS无actual — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0234）",fix="补actual"))
        BugDB.load(BugRule(id="KT-2560",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="commonTest中使用平台API",trigger="assertEquals(System.currentTimeMillis(),ts)",detection="平台特定 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0235）",fix="expect/actual测试"))
        ...
        BugDB.load(BugRule(id="KT-2575",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
            title="iOS与JVM路径分隔符",trigger="File(\\\"a/b\\\")在iOS",detection="平台路径差异 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0119）",fix="expect/actual或Path"))
        BugDB.load(BugRule(id="KT-2576",category=BugCategory.KMP,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2578",category=BugCategory.KMP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2585",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2586",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2587",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="自引用属性（Char）",trigger="val x:Char=x+1",detection="循环引用 — Char类型字符编码边界,正则匹配组越界（参见 KT-0125）",fix="用by lazy或初始化块"))
        BugDB.load(BugRule(id="KT-2588",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Double?）",trigger="val x:Double?=\\\"hi\\\"",detection="已知String — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0126）",fix="自动推断"))
        BugDB.load(BugRule(id="KT-2589",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2590",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2591",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2594",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2595",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2596",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2599",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2601",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2602",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2603",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2604",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="误把AI的建议当编译器报错",trigger="AI说你的代码有bug但编译器编译过了",detection="区分AI瞎说和编译器 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0281）",fix="先编译再采纳"))
        BugDB.load(BugRule(id="KT-2605",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2606",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2609",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2610",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2611",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2612",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2613",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2614",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2617",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2620",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2622",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Int版）（Any）",trigger="val x:Any=\\\\\\\"hi\\\\\\\"",detection="已知Int — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0332）",fix="自动推断"))
        BugDB.load(BugRule(id="KT-2623",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Long版）",trigger="val x:Long=\\\\\\\"hi\\\\\\\"",detection="已知Long — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0333）",fix="自动推断"))
        BugDB.load(BugRule(id="KT-2624",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Double版）",trigger="val x:Double=\\\\\\\"hi\\\\\\\"",detection="已知Double — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0334）",fix="自动推断"))
        BugDB.load(BugRule(id="KT-2625",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注（Boolean版）",trigger="val x:Boolean=\\\\\\\"hi\\\\\\\"",detection="已知Boolean — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0335）",fix="自动推断"))
        BugDB.load(BugRule(id="KT-2626",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="默认参数+扩展函数+泛型=三歧义（Long版）（Any?）",trigger="fun <T> Any?<T>.f(n:Long=1){};listOf(1).f()",detection="扩展接收者+泛型+默认参数解析 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0339）",fix="显式传参"))
        ...
        BugDB.load(BugRule(id="KT-2645",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="类型推断歧义",trigger="val x=if(cond) 1 else null",detection="推断为Int? — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0122）",fix="显式标注类型"))
        BugDB.load(BugRule(id="KT-2646",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="Nothing类型传染",trigger="fun e():Nothing=throw E();val x=e()",detection="x类型为Nothing — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0123）",fix="显式类型标注"))
        BugDB.load(BugRule(id="KT-2647",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="Lambda返回类型推断错误",trigger="val f={if(x) 1 else \\\"hi\\\"}",detection="推断为Any — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0124）",fix="显式返回类型"))
        BugDB.load(BugRule(id="KT-2648",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="自引用属性（Any?）",trigger="val x:Any?=x+1",detection="循环引用 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0125）",fix="用by lazy或初始化块"))
        BugDB.load(BugRule(id="KT-2649",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="不必要的类型标注",trigger="val x:String=\\\"hi\\\"",detection="已知String — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0126）",fix="自动推断"))
        BugDB.load(BugRule(id="KT-2650",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="SAM+overload解析",trigger="fun f(r:Runnable);fun f(c:Callable);f{println()}",detection="Java SAM转换歧义 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0237）",fix="显式类型"))
        BugDB.load(BugRule(id="KT-2651",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="data class在when中穷举",trigger="when(val d=dataClass(...)){Data(1)->1}",detection="检查所有字段 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0238）",fix="if-else替代"))
        BugDB.load(BugRule(id="KT-2652",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="Lambda返回值return+非Lambda",trigger="fun f()=lambda{return 1}",detection="return在lambda非法 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0239）",fix="return@lambda"))
        BugDB.load(BugRule(id="KT-2653",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="空catch块",trigger="try{risky()}catch(e:Exception){}",detection="吞掉所有异常 — Char类型字符编码边界,正则匹配组越界（参见 KT-0240）",fix="至少日志记录"))
        BugDB.load(BugRule(id="KT-2654",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="unsafe cast+泛型",trigger="fun <T> f(a:Any)=a as T",detection="擦除后不安全 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0241）",fix="pass reified+"))
        BugDB.load(BugRule(id="KT-2655",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="lazy初始化捕获可变引用",trigger="var x=0;val y by lazy{x}",detection="捕获的是var引用 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0242）",fix="val x=0先"))
        BugDB.load(BugRule(id="KT-2656",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="函数引用+重载（Float）",trigger="fun f(i:Float){};fun f(s:Float){};val r=::f",detection="歧义 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0243）",fix="指定类型:(Int)->Unit"))
        BugDB.load(BugRule(id="KT-2657",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="编译器编译自己源码时卡死",trigger="kotlin-head编译Main.kt自身",detection="自引用类型推断循环 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0261）",fix="分离编译阶段"))
        BugDB.load(BugRule(id="KT-2658",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="BugScanner扫描到自己",trigger="BugDB.scan(BugScanner的源码)",detection="自检逻辑循环 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0262）",fix="元规则豁免self-scan"))
        BugDB.load(BugRule(id="KT-2659",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="LiveDeclarationGraph自引用死循环",trigger="class A(val a:A)",detection="声明依赖自身 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0263）",fix="循环引用检测"))
        BugDB.load(BugRule(id="KT-2660",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="Parser遇到文件名为hell时倾全军之力",trigger="hell.kt触发isHostileFile=true;所有资源耗尽",detection="地狱文件识别过激 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0264）",fix="只加倍不倾全军"))
        BugDB.load(BugRule(id="KT-2661",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="反编译管线反编译自己",trigger="jadx反编译kotlin-head.jar再编译",detection="语义递归退化 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0265）",fix="跳过自身jar"))
        BugDB.load(BugRule(id="KT-2662",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="Kotlin T!类型让编译器以为非空实则null（Double）",trigger="val x=javaGet();编译器推断Double;运行时NPE",detection="T!被不当优化 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0266）",fix="显式标注String?"))
        BugDB.load(BugRule(id="KT-2663",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="Nothing类型吞噬所有代码",trigger="fun e():Nothing=throw E();e().also{unreachable()}",detection="Nothing传染正常路径 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0267）",fix="不链式调用Nothing"))
        BugDB.load(BugRule(id="KT-2664",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="类型推断选了谁都没想到的类型（Set<Int>）",trigger="listOf(1)与emptySet<Int>()合并推断Set<Int><Set<Int>>?",detection="多个候选时选最意外的 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0268）",fix="显式泛型参数"))
        BugDB.load(BugRule(id="KT-2665",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="误把AI的建议当编译器报错",trigger="AI说你的代码有bug但编译器编译过了",detection="区分AI瞎说和编译器 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0281）",fix="先编译再采纳"))
        BugDB.load(BugRule(id="KT-2666",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MILD,
            title="注释里写TODO导致被BugScanner报bug",trigger="// TODO: fix this → BugDB hit",detection="TODO被当bug模式 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0282）",fix="BugDB加-TODO排除规则"))
        BugDB.load(BugRule(id="KT-2667",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="编译器优化阶段把正确代码优化成错误代码",trigger="Pass.inline错误展开导致语义变化",detection="Pass副作用 — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0290）",fix="Pass后加语义等价校验"))
        BugDB.load(BugRule(id="KT-2668",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="类型推断在递归函数中选择最窄类型",trigger="fun f()=if(cond) f() else 0→Int",detection="递归基推断为Int但期望Long — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0294）",fix="显式返回类型"))
        BugDB.load(BugRule(id="KT-2669",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="默认参数+扩展函数+泛型=三歧义（Short）",trigger="fun <T> Short<T>.f(n:Short=1){};listOf(1).f()",detection="扩展接收者+泛型+默认参数解析 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0309）",fix="显式传参"))
        BugDB.load(BugRule(id="KT-2670",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.MODERATE,
            title="把launch当async用等不到结果",trigger="val res=launch{calc()};println(res)",detection="launch返回Job不是Deferred — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0318）",fix="需要结果用async+await"))
        BugDB.load(BugRule(id="KT-2671",category=BugCategory.COMPILER_TRAP,severity=BugSeverity.SEVERE,
            title="BugDB被设为主入口启动了但不编译只报告bug",trigger="fun main()=BugDB.scan(args);编译结果变成了bug报告",detection="身份混淆：工具当程序 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0319）",fix="main调compile不是scan"))
        BugDB.load(BugRule(id="KT-2672",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2673",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="不必要装箱（Byte）",trigger="val x:Byte?=42;x?.let{",detection="Int?装箱 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0128）",fix="避免可空"))
        BugDB.load(BugRule(id="KT-2674",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2676",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的对象创建",trigger="for(i in 1..n){Regex(\\\"\\\\\\\\d\\\").findAll(s)}",detection="反复编译正则 — Char类型字符编码边界,正则匹配组越界（参见 KT-0131）",fix="提取到循环外"))
        BugDB.load(BugRule(id="KT-2677",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2678",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2679",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2680",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2681",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2682",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2683",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2684",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2685",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2686",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2687",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2688",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2690",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2701",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="不必要装箱（Float）",trigger="val x:Float?=42;x?.let{",detection="Int?装箱 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0128）",fix="避免可空"))
        BugDB.load(BugRule(id="KT-2702",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="高阶函数链过多",trigger="list.filter{.map{.flatMap{",detection="多次遍历 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0129）",fix="合并为单次fold"))
        BugDB.load(BugRule(id="KT-2703",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用sequence",trigger="list.map{.filter{.first()",detection="中间集合 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0130）",fix="asSequence"))
        BugDB.load(BugRule(id="KT-2704",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的对象创建",trigger="for(i in 1..n){Regex(\\\"\\\\\\\\d\\\").findAll(s)}",detection="反复编译正则 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0131）",fix="提取到循环外"))
        BugDB.load(BugRule(id="KT-2705",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="varargs传递数组（Int?）",trigger="fun f(vararg s:Int??);f(arrayOf(\\\"a\\\"))",detection="需要展开操作符 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0132）",fix="f(*arrayOf)"))
        BugDB.load(BugRule(id="KT-2706",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="N+1查询在集合操作",trigger="users.forEach{u->db.query(u.id)}",detection="每项一次DB — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0244）",fix="batch查询"))
        BugDB.load(BugRule(id="KT-2707",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="频繁创建SimpleDateFormat",trigger="for(i in 1..1000){SimpleDateFormat(\\\"yyyy\\\").parse(s)}",detection="线程不安全+开销 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0245）",fix="java.time或DateTimeFormatter"))
        BugDB.load(BugRule(id="KT-2708",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用@Stable/@Immutable（String?）",trigger="data class D(val x:String?,val y:String?)",detection="Compose重组优化 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0246）",fix="加@Immutable"))
        BugDB.load(BugRule(id="KT-2709",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的LazyThreadSafetyMode",trigger="val x by lazy(LazyThreadSafetyMode.SYNCHRONIZED){42}",detection="常量不需要同步 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0247）",fix="NONE"))
        BugDB.load(BugRule(id="KT-2710",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="byte数组频繁copy",trigger="for(chunk in stream){buf.copyOfRange()}",detection="每次新建数组 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0248）",fix="复用buffer"))
        BugDB.load(BugRule(id="KT-2711",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="EventBus频道互相触发死循环",trigger="EventBus.emit('a',ev);subscribe('a'){emit('b',ev)};subscribe('b'){emit('a',ev)}",detection="频道AB互相触发 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0270）",fix="事件去重+深度限制"))
        BugDB.load(BugRule(id="KT-2712",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="lazy初始化循环依赖",trigger="val a by lazy{b};val b by lazy{a}",detection="两个lazy互相等 — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0271）",fix="break one with lateinit"))
        BugDB.load(BugRule(id="KT-2713",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="inline函数互相展开到JVM 64KB限制",trigger="inline fun a(){b()};inline fun b(){a()}",detection="互相inline展开致字节码超限 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0272）",fix="一个去掉inline"))
        BugDB.load(BugRule(id="KT-2714",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="大量BugDB规则扫描空文件",trigger="BugDB.scan(\\\"\\\")",detection="全索引扫描空字符串 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0273）",fix="空字符串短路返回"))
        BugDB.load(BugRule(id="KT-2715",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="10MB的JSON被当源码读入",trigger="val src=File('10mb.json').readText();compile(src)",detection="非Kotlin文件被误读 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0283）",fix="文件扩展名检查"))
        BugDB.load(BugRule(id="KT-2716",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="APK当JSON解析不报错只返回空",trigger="JsonUtil.decode(apkBytes)",detection="不抛异常只静默 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0284）",fix="检查Content-Type或magic bytes"))
        BugDB.load(BugRule(id="KT-2717",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="路标指针指向null却认为有值",trigger="LiveDeclarationGraph.getNode返回null但下游当非null用",detection="假指针 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0289）",fix="getNode后判null"))
        BugDB.load(BugRule(id="KT-2718",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="asSequence+first+重复调用=每次重新求值",trigger="val seq=list.asSequence().filter{;seq.first();seq.first()",detection="sequence每次first重新求值 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0303）",fix="先toList再取"))
        BugDB.load(BugRule(id="KT-2719",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="inline+递归泛型=编译时间爆炸（Sequence<Long>）",trigger="inline fun <reified T> f(n:Sequence<Long>){if(n>0)f<T>(n-1)}",detection="inline+递归=每层展开 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0308）",fix="去掉inline或用while"))
        BugDB.load(BugRule(id="KT-2720",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="parallelStream在ForkJoinPool里反而串行",trigger="list.parallelStream().map{sleep(100);it}.collect()",detection="公共池被其他任务占满 — Char类型字符编码边界,正则匹配组越界（参见 KT-0316）",fix="自定义ForkJoinPool"))
        ...
        BugDB.load(BugRule(id="KT-2729",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="不必要装箱（Double）",trigger="val x:Double?=42;x?.let{",detection="Int?装箱 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0128）",fix="避免可空"))
        BugDB.load(BugRule(id="KT-2730",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="高阶函数链过多",trigger="list.filter{.map{.flatMap{",detection="多次遍历 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0129）",fix="合并为单次fold"))
        BugDB.load(BugRule(id="KT-2731",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用sequence",trigger="list.map{.filter{.first()",detection="中间集合 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0130）",fix="asSequence"))
        BugDB.load(BugRule(id="KT-2732",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的对象创建",trigger="for(i in 1..n){Regex(\\\"\\\\\\\\d\\\").findAll(s)}",detection="反复编译正则 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0131）",fix="提取到循环外"))
        BugDB.load(BugRule(id="KT-2733",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="varargs传递数组（Any）",trigger="fun f(vararg s:Any);f(arrayOf(\\\"a\\\"))",detection="需要展开操作符 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0132）",fix="f(*arrayOf)"))
        BugDB.load(BugRule(id="KT-2734",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="N+1查询在集合操作",trigger="users.forEach{u->db.query(u.id)}",detection="每项一次DB — List<String>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0244）",fix="batch查询"))
        BugDB.load(BugRule(id="KT-2735",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="频繁创建SimpleDateFormat",trigger="for(i in 1..1000){SimpleDateFormat(\\\"yyyy\\\").parse(s)}",detection="线程不安全+开销 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0245）",fix="java.time或DateTimeFormatter"))
        BugDB.load(BugRule(id="KT-2736",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用@Stable/@Immutable（Short）",trigger="data class D(val x:Short,val y:Short)",detection="Compose重组优化 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0246）",fix="加@Immutable"))
        BugDB.load(BugRule(id="KT-2737",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的LazyThreadSafetyMode",trigger="val x by lazy(LazyThreadSafetyMode.SYNCHRONIZED){42}",detection="常量不需要同步 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0247）",fix="NONE"))
        BugDB.load(BugRule(id="KT-2738",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="byte数组频繁copy",trigger="for(chunk in stream){buf.copyOfRange()}",detection="每次新建数组 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0248）",fix="复用buffer"))
        BugDB.load(BugRule(id="KT-2739",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="EventBus频道互相触发死循环",trigger="EventBus.emit('a',ev);subscribe('a'){emit('b',ev)};subscribe('b'){emit('a',ev)}",detection="频道AB互相触发 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0270）",fix="事件去重+深度限制"))
        BugDB.load(BugRule(id="KT-2740",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="lazy初始化循环依赖",trigger="val a by lazy{b};val b by lazy{a}",detection="两个lazy互相等 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0271）",fix="break one with lateinit"))
        BugDB.load(BugRule(id="KT-2741",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="inline函数互相展开到JVM 64KB限制",trigger="inline fun a(){b()};inline fun b(){a()}",detection="互相inline展开致字节码超限 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0272）",fix="一个去掉inline"))
        BugDB.load(BugRule(id="KT-2742",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="大量BugDB规则扫描空文件",trigger="BugDB.scan(\\\"\\\")",detection="全索引扫描空字符串 — Char类型字符编码边界,正则匹配组越界（参见 KT-0273）",fix="空字符串短路返回"))
        BugDB.load(BugRule(id="KT-2743",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="10MB的JSON被当源码读入",trigger="val src=File('10mb.json').readText();compile(src)",detection="非Kotlin文件被误读 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0283）",fix="文件扩展名检查"))
        BugDB.load(BugRule(id="KT-2744",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="APK当JSON解析不报错只返回空",trigger="JsonUtil.decode(apkBytes)",detection="不抛异常只静默 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0284）",fix="检查Content-Type或magic bytes"))
        BugDB.load(BugRule(id="KT-2745",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="路标指针指向null却认为有值",trigger="LiveDeclarationGraph.getNode返回null但下游当非null用",detection="假指针 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0289）",fix="getNode后判null"))
        BugDB.load(BugRule(id="KT-2746",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="asSequence+first+重复调用=每次重新求值",trigger="val seq=list.asSequence().filter{;seq.first();seq.first()",detection="sequence每次first重新求值 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0303）",fix="先toList再取"))
        BugDB.load(BugRule(id="KT-2747",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="inline+递归泛型=编译时间爆炸（MutableList<Double>）",trigger="inline fun <reified T> f(n:MutableMutableList<Double><Double>){if(n>0)f<T>(n-1)}",detection="inline+递归=每层展开 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0308）",fix="去掉inline或用while"))
        BugDB.load(BugRule(id="KT-2748",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="parallelStream在ForkJoinPool里反而串行",trigger="list.parallelStream().map{sleep(100);it}.collect()",detection="公共池被其他任务占满 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0316）",fix="自定义ForkJoinPool"))
        ...
        BugDB.load(BugRule(id="KT-2757",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="不必要装箱",trigger="val x:Int?=42;x?.let{",detection="Int?装箱 — Int类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0128）",fix="避免可空"))
        BugDB.load(BugRule(id="KT-2758",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="高阶函数链过多",trigger="list.filter{.map{.flatMap{",detection="多次遍历 — Short类型端口号溢出,旧协议数据长度字段（参见 KT-0129）",fix="合并为单次fold"))
        BugDB.load(BugRule(id="KT-2759",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用sequence",trigger="list.map{.filter{.first()",detection="中间集合 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0130）",fix="asSequence"))
        BugDB.load(BugRule(id="KT-2760",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的对象创建",trigger="for(i in 1..n){Regex(\\\"\\\\\\\\d\\\").findAll(s)}",detection="反复编译正则 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0131）",fix="提取到循环外"))
        BugDB.load(BugRule(id="KT-2761",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="varargs传递数组（Byte）",trigger="fun f(vararg s:Byte);f(arrayOf(\\\"a\\\"))",detection="需要展开操作符 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0132）",fix="f(*arrayOf)"))
        BugDB.load(BugRule(id="KT-2762",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="N+1查询在集合操作",trigger="users.forEach{u->db.query(u.id)}",detection="每项一次DB — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0244）",fix="batch查询"))
        BugDB.load(BugRule(id="KT-2763",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="频繁创建SimpleDateFormat",trigger="for(i in 1..1000){SimpleDateFormat(\\\"yyyy\\\").parse(s)}",detection="线程不安全+开销 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0245）",fix="java.time或DateTimeFormatter"))
        BugDB.load(BugRule(id="KT-2764",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用@Stable/@Immutable（Char）",trigger="data class D(val x:Char,val y:Char)",detection="Compose重组优化 — Char类型字符编码边界,正则匹配组越界（参见 KT-0246）",fix="加@Immutable"))
        BugDB.load(BugRule(id="KT-2765",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的LazyThreadSafetyMode",trigger="val x by lazy(LazyThreadSafetyMode.SYNCHRONIZED){42}",detection="常量不需要同步 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0247）",fix="NONE"))
        BugDB.load(BugRule(id="KT-2766",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="byte数组频繁copy",trigger="for(chunk in stream){buf.copyOfRange()}",detection="每次新建数组 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0248）",fix="复用buffer"))
        BugDB.load(BugRule(id="KT-2767",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="EventBus频道互相触发死循环",trigger="EventBus.emit('a',ev);subscribe('a'){emit('b',ev)};subscribe('b'){emit('a',ev)}",detection="频道AB互相触发 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0270）",fix="事件去重+深度限制"))
        BugDB.load(BugRule(id="KT-2768",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="lazy初始化循环依赖",trigger="val a by lazy{b};val b by lazy{a}",detection="两个lazy互相等 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0271）",fix="break one with lateinit"))
        BugDB.load(BugRule(id="KT-2769",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="inline函数互相展开到JVM 64KB限制",trigger="inline fun a(){b()};inline fun b(){a()}",detection="互相inline展开致字节码超限 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0272）",fix="一个去掉inline"))
        BugDB.load(BugRule(id="KT-2770",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="大量BugDB规则扫描空文件",trigger="BugDB.scan(\\\"\\\")",detection="全索引扫描空字符串 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0273）",fix="空字符串短路返回"))
        BugDB.load(BugRule(id="KT-2771",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="10MB的JSON被当源码读入",trigger="val src=File('10mb.json').readText();compile(src)",detection="非Kotlin文件被误读 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0283）",fix="文件扩展名检查"))
        BugDB.load(BugRule(id="KT-2772",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="APK当JSON解析不报错只返回空",trigger="JsonUtil.decode(apkBytes)",detection="不抛异常只静默 — Map<String,Int>类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0284）",fix="检查Content-Type或magic bytes"))
        BugDB.load(BugRule(id="KT-2773",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="路标指针指向null却认为有值",trigger="LiveDeclarationGraph.getNode返回null但下游当非null用",detection="假指针 — Double类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0289）",fix="getNode后判null"))
        BugDB.load(BugRule(id="KT-2774",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="asSequence+first+重复调用=每次重新求值",trigger="val seq=list.asSequence().filter{;seq.first();seq.first()",detection="sequence每次first重新求值 — String?类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0303）",fix="先toList再取"))
        BugDB.load(BugRule(id="KT-2775",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="inline+递归泛型=编译时间爆炸（Set<Int>）",trigger="inline fun <reified T> f(n:Set<Int>){if(n>0)f<T>(n-1)}",detection="inline+递归=每层展开 — Set<Int>类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0308）",fix="去掉inline或用while"))
        BugDB.load(BugRule(id="KT-2776",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="parallelStream在ForkJoinPool里反而串行",trigger="list.parallelStream().map{sleep(100);it}.collect()",detection="公共池被其他任务占满 — Long类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0316）",fix="自定义ForkJoinPool"))
        ...
        BugDB.load(BugRule(id="KT-2785",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="不必要装箱（Sequence<Long>）",trigger="val x:Sequence<Long>?=42;x?.let{",detection="Int?装箱 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0128）",fix="避免可空"))
        BugDB.load(BugRule(id="KT-2786",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="高阶函数链过多",trigger="list.filter{.map{.flatMap{",detection="多次遍历 — Char类型字符编码边界,正则匹配组越界（参见 KT-0129）",fix="合并为单次fold"))
        BugDB.load(BugRule(id="KT-2787",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用sequence",trigger="list.map{.filter{.first()",detection="中间集合 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0130）",fix="asSequence"))
        BugDB.load(BugRule(id="KT-2788",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的对象创建",trigger="for(i in 1..n){Regex(\\\"\\\\\\\\d\\\").findAll(s)}",detection="反复编译正则 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0131）",fix="提取到循环外"))
        BugDB.load(BugRule(id="KT-2789",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="varargs传递数组（Float）",trigger="fun f(vararg s:Float);f(arrayOf(\\\"a\\\"))",detection="需要展开操作符 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0132）",fix="f(*arrayOf)"))
        BugDB.load(BugRule(id="KT-2790",category=BugCategory.PERFORMANCE,severity=BugSeverity.SEVERE,
            title="N+1查询在集合操作",trigger="users.forEach{u->db.query(u.id)}",detection="每项一次DB — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0244）",fix="batch查询"))
        BugDB.load(BugRule(id="KT-2791",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="频繁创建SimpleDateFormat",trigger="for(i in 1..1000){SimpleDateFormat(\\\"yyyy\\\").parse(s)}",detection="线程不安全+开销 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0245）",fix="java.time或DateTimeFormatter"))
        BugDB.load(BugRule(id="KT-2792",category=BugCategory.PERFORMANCE,severity=BugSeverity.MODERATE,
            title="未使用@Stable/@Immutable（Boolean）",trigger="data class D(val x:Boolean,val y:Boolean)",detection="Compose重组优化 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0246）",fix="加@Immutable"))
        BugDB.load(BugRule(id="KT-2793",category=BugCategory.PERFORMANCE,severity=BugSeverity.MILD,
            title="不必要的LazyThreadSafetyMode",trigger="val x by lazy(LazyThreadSafetyMode.SYNCHRONIZED){42}",detection="常量不需要同步 — Int?类型Map.get返回值,SharedPreferences读取,DB自增ID溢出（参见 KT-0247）",fix="NONE"))
        ...
        BugDB.load(BugRule(id="KT-2795",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2798",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="未验证SSL证书",trigger="trustAllCerts()",detection="中间人攻击 — Char类型字符编码边界,正则匹配组越界（参见 KT-0137）",fix="证书固定"))
        BugDB.load(BugRule(id="KT-2799",category=BugCategory.SECURITY,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2800",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2802",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2803",category=BugCategory.SECURITY,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2804",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2816",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
            title="SQL注入拼接",trigger="db.execSQL(\\\"SELECT * FROM u WHERE n='\\\$name'\\\")",detection="直接拼接 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0134）",fix="参数化查询"))
        BugDB.load(BugRule(id="KT-2817",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="日志泄露敏感信息",trigger="Log.d(\\\"TAG\\\",\\\"token=\\\$token\\\")",detection="日志可被读取 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0135）",fix="脱敏"))
        BugDB.load(BugRule(id="KT-2818",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="明文存储密码（Boolean?）",trigger="prefs.edit().putBoolean?(\\\"pwd\\\",password).apply()",detection="SharedPreferences明文 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0136）",fix="EncryptedSharedPreferences"))
        BugDB.load(BugRule(id="KT-2819",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="未验证SSL证书",trigger="trustAllCerts()",detection="中间人攻击 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0137）",fix="证书固定"))
        BugDB.load(BugRule(id="KT-2820",category=BugCategory.SECURITY,severity=BugSeverity.MILD,
            title="WebView JavaScript启用",trigger="webView.settings.javaScriptEnabled=true",detection="XSS风险 — Char类型字符编码边界,正则匹配组越界（参见 KT-0138）",fix="禁用或白名单"))
        BugDB.load(BugRule(id="KT-2821",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
            title="Intent extras明文传敏感数据",trigger="intent.putExtra(\\\"token\\\",token)",detection="可被其他app读取 — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0249）",fix="加密或避免"))
        BugDB.load(BugRule(id="KT-2822",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="WebView.addJavascriptInterface（Array<Boolean>）",trigger="webView.addJavascriptArray<Boolean>erface(obj,\\\"android\\\")",detection="JS可调用Java — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0250）",fix="@JavascriptInterface仅暴露必要方法"))
        BugDB.load(BugRule(id="KT-2823",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="FileProvider路径遍历",trigger="FileProvider.getUriForFile(ctx,path)",detection="任意文件泄露 — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0251）",fix="限制根目录"))
        BugDB.load(BugRule(id="KT-2824",category=BugCategory.SECURITY,severity=BugSeverity.MILD,
            title="日志使用e.printStackTrace",trigger="e.printStackTrace()",detection="泄露到logcat — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0252）",fix="Log.e(TAG,\\\"msg\\\",e)"))
        BugDB.load(BugRule(id="KT-2825",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
            title="debug日志里打印完整银行卡号",trigger="if(BuildConfig.DEBUG)Log.d('PAY',cardNumber)",detection="debug模式泄露 — MutableList<Double>类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0276）",fix="release不输出或脱敏"))
        ...
        BugDB.load(BugRule(id="KT-2837",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
            title="SQL注入拼接",trigger="db.execSQL(\\\"SELECT * FROM u WHERE n='\\\$name'\\\")",detection="直接拼接 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0134）",fix="参数化查询"))
        BugDB.load(BugRule(id="KT-2838",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="日志泄露敏感信息",trigger="Log.d(\\\"TAG\\\",\\\"token=\\\$token\\\")",detection="日志可被读取 — String类型Java互操作T!,JSON缺字段,DB nullable列（参见 KT-0135）",fix="脱敏"))
        BugDB.load(BugRule(id="KT-2839",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="明文存储密码（Byte）",trigger="prefs.edit().putByte(\\\"pwd\\\",password).apply()",detection="SharedPreferences明文 — Byte类型二进制协议解析,文件I/O偏移量,位运算掩码（参见 KT-0136）",fix="EncryptedSharedPreferences"))
        BugDB.load(BugRule(id="KT-2840",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="未验证SSL证书",trigger="trustAllCerts()",detection="中间人攻击 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0137）",fix="证书固定"))
        BugDB.load(BugRule(id="KT-2841",category=BugCategory.SECURITY,severity=BugSeverity.MILD,
            title="WebView JavaScript启用",trigger="webView.settings.javaScriptEnabled=true",detection="XSS风险 — Sequence<Long>类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0138）",fix="禁用或白名单"))
        BugDB.load(BugRule(id="KT-2842",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
            title="Intent extras明文传敏感数据",trigger="intent.putExtra(\\\"token\\\",token)",detection="可被其他app读取 — Char类型字符编码边界,正则匹配组越界（参见 KT-0249）",fix="加密或避免"))
        BugDB.load(BugRule(id="KT-2843",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="WebView.addJavascriptInterface（Double?）",trigger="webView.addJavascriptDouble?erface(obj,\\\"android\\\")",detection="JS可调用Java — Double?类型浮点精度丢失,JSON数字类型模糊,累积误差（参见 KT-0250）",fix="@JavascriptInterface仅暴露必要方法"))
        BugDB.load(BugRule(id="KT-2844",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="FileProvider路径遍历",trigger="FileProvider.getUriForFile(ctx,path)",detection="任意文件泄露 — Array<Boolean>类型三态逻辑误用,配置项缺失默认值（参见 KT-0251）",fix="限制根目录"))
        BugDB.load(BugRule(id="KT-2845",category=BugCategory.SECURITY,severity=BugSeverity.MILD,
            title="日志使用e.printStackTrace",trigger="e.printStackTrace()",detection="泄露到logcat — Float类型精度低于Double,GPU纹理坐标,传感器数据（参见 KT-0252）",fix="Log.e(TAG,\\\"msg\\\",e)"))
        BugDB.load(BugRule(id="KT-2846",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
            title="debug日志里打印完整银行卡号",trigger="if(BuildConfig.DEBUG)Log.d('PAY',cardNumber)",detection="debug模式泄露 — Long?类型时间戳边界,ID生成器溢出,计数器超限（参见 KT-0276）",fix="release不输出或脱敏"))
        ...
        BugDB.load(BugRule(id="KT-2859",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
            title="日志泄露敏感信息",trigger="Log.d(\\\"TAG\\\",\\\"token=\\\$token\\\")",detection="日志可被读取 — Any?类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0135）",fix="脱敏"))
        BugDB.load(BugRule(id="KT-2860",category=BugCategory.SECURITY,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2863",category=BugCategory.SECURITY,severity=BugSeverity.SEVERE,
        ...
        BugDB.load(BugRule(id="KT-2866",category=BugCategory.SECURITY,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2875",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
            title="文件分隔符硬编码",trigger="File(\\\"a/b\\\").path在Windows",detection="\\\\差异 — Boolean?类型三态逻辑误用,配置项缺失默认值（参见 KT-0141）",fix="File.separator或Path"))
        BugDB.load(BugRule(id="KT-2876",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2878",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2890",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
            title="文件分隔符硬编码",trigger="File(\\\"a/b\\\").path在Windows",detection="\\\\差异 — Any类型类型擦除信息丢失,动态代理返回,反射调用（参见 KT-0141）",fix="File.separator或Path"))
        BugDB.load(BugRule(id="KT-2891",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2893",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2905",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
            title="文件分隔符硬编码",trigger="File(\\\"a/b\\\").path在Windows",detection="\\\\差异 — Boolean类型三态逻辑误用,配置项缺失默认值（参见 KT-0141）",fix="File.separator或Path"))
        BugDB.load(BugRule(id="KT-2906",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2908",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2920",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2921",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MODERATE,
        ...
        BugDB.load(BugRule(id="KT-2923",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2935",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MILD,
        ...
        BugDB.load(BugRule(id="KT-2936",category=BugCategory.MULTIPLATFORM,severity=BugSeverity.MODERATE,
