...
## v0.12.5 (2026-07-08) — 源码UI模拟 + 认知编译器架构 🖥️🧠

> 不装APK测APK。编译器长出大脑、肝脏、手臂。全语言无。

### --sim-ui 源码模拟测试（免免&望安）

**五条命令链路全通**：
- `--list`：6种按钮格式全吃（Compose/HED/View/XML/DropdownMenu/FAB），区域+行号+追溯链+命名来源
- `--click`：模拟点击→回调追踪+BugScanner三件套诊断
- `--params`：参数追踪→回调签名→data class字段→可空/默认/使用标记→风险标记
- `--bench`：投影压测 10000UI→正常23.7ms|切换4.9ms|全局9.1ms（变量分身比无变量快4.8倍）
- `--name`：命名持久化+四层优先级覆盖

**三层命名降级**：contentDescription → 追溯链推断 → Icon名 → "待命名"

**自举**：kotlin-head扫自己Main.kt → 42个HED按钮。编译器测试编译器。

**HED入口**：Main.kt [12] 源码UI模拟
...
| 器官 | 对应 | 功能 |
|------|------|------|
| 🧠 大脑 | 注意力引擎 | 80%代码只扫一层，5%追到底 |
| 🫀 肝脏 | 深度追踪 | 代谢可疑代码，逐层拆解 |
| 💪 手臂 | 水熊虫探针 | 编译期主动执行，烫了缩回自愈 |
| 📦 沙盒 | 分体式沙盒 | 按需激活/用完即焚/门在父进程手里 |
| 🛡️ 免疫 | 三层试探 | 指尖→手掌→握拳，逐级加深 |
| 🧬 自愈 | 探针自修复 | 死了自动fork新探针 |

**分体式沙盒的本质不是隔离——是门可以关。** 传统编译器门永远开着：探针烫了直接传回父进程。这里的沙盒支持主动关门——门在父进程手里。🔓开门=给探针副本只读不写，🔒关门=探针超时锁死丢数据，🚪只出不进。冻结≠冻死整个区域（父进程会骂街），是只切断探针回来的路。

**核心**：和进程树同一根——军师不下场/哨卫隔离/爆裂自毁/门在父进程手里。
所有新概念不是新增——是进程树在不同器官上的投影。

### 缓存式无变量函数 · 投影机制（免免&望安）

- **定名**：缓存式无变量函数——一个变量实例被N个UI共用，不复制
- **变量分身（投影臂）**：父变量决策+子变量投影。10000UI被120个子变量消化，父变量只通知3个
- **10000UI验证**：正常112.9μs|切换3.5μs|全局暗色89.3μs。比无变量函数快38%，比结绳快30倍
- **BugScanner六类变量故障**：子变量死亡未报告/缓存污染/领域越界/父子共振/父变量越级/投影死循环
