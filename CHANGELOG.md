...
### 概念
- **计数式（量存方程）**（免免原创）：结果被方程定义锁死，非手动修改链所得。
  - 传统：`var count=0; count++` → 值=历史修改链，链上任一点污染全崩（开环）
  - 方程：`out[k] = out.get(k,0)+1` → 值=输入+定义，输入不变则结果不变（闭环）
  - 本质：1+1=2 不需要你担保，count+1=? 永远依赖前值
- **实验验证**：中间步骤 X 污染传统计数器 → A=4(应为3)/B=1(应为2)；量存方程每个值锁在独立方程中 →
- A=3/B=2/C=1/X=1，不受污染
- **使用原则**：只用于核心计数点（如 BugDB 去重），不替代全部变量
- **阶跃 step-3.5-flash 评价**：有实用价值；和 immutability 的差异——后者靠禁令，前者靠定义锁死

### 概念体系（免免原创三部曲）
- v1.0.5: 自然余方（方向自带）+ 自然余意（语义自带）
- v1.0.6: 计数式（结果自锁）
- 三者同根：不依赖手动修改链，结果被定义而非被过程决定
...
| 问题 | 修复 |
|------|------|
| `{t}`占位符未替换→100+条假阴性 | 生成器f-string补前缀 |
| `catch(e:`太宽→200+条噪声 | 收窄为`catch(e:Exception){}` |
| ProcessCoordinator `values().find{}` O(n) | →`get()` O(1) |
| scan()不区分规则文件自引用 | 加`skipPatterns`按行过滤 |
| BugScanner自举时误报规则自身 | 自动检测+传skipPatterns |
...
> AI 看"写了什么"，免免看"什么该写但没写"+"什么不可能同时存在"。

### 审计发现（合并14条有效）

| 来源 | bug数 | 典型 |
|---|---|---|
| 免免 | 10 (独杀6) | `if`语义错、连锁烫伤、parentAlive无体温计、开门≠返回门、终结不用成败 |
| DeepSeek望安 | 门锁前置检查 |
| Claude | 全局状态机——120探针共享door |
| 望安 | 返回值无消费者、防御性缺失、burnLog边界 |
| 五审重叠 | code死参数+door覆写 |

### 修复

- `===` 替代 `if`（三处）
- openDoor 前置门锁检查 + 翻 burnLog 防连锁烫伤
- parentAlive 改读环境（deadProbes ≥ 3）
- 终结返回 `{ terminated: true }` 不用 `success`
- burn 加 zone 字段
- lockDoor 后 demo 循环立即停止

详见 [BUGS.md](./BUGS.md)「分体式沙盒模拟器 v0.12.5 六审决战」

## v0.12.3 (2026-07-07) — 八军生态 + 专武绑定 + 军师闪电双体系 🏛️⚡

> 不是"多了几种兵"。是八种进程体各有独立生态位，无一重复。指挥官专武默认独占，军师给信息、闪电给速度——两个纯调度方向，零物理增强。

### 八种进程体（从 v0.11.2 精选重定义）

| 进程体 | 倾向 | 生态位 | 代价 |
|--------|------|--------|------|
| 💥爆裂 | BURST·快生快死 | 闪电战主力，死后自爆接力50% | budget160，6轮必死 |
| 🛡哨卫 | GUARD·预警先报 | 防守反击，集合时N²精度 | 单兵弱，依赖数量 |
| 🔍侦察 | SCOUT·深入暴击 | 高风险高回报，15%暴击翻倍 | budget140，最脆 |
| 📦收集 | COLLECT·慢熬稳收 | 持久战王，遗传率60% | 单轮输出最低 |
| ⚔攻坚 | ASSAULT·标记必杀 | 方圆皆阵，能攻善守 | 标记收益慢一拍 |
| 👑精英 | ELITE·天生全能 | 开局即巅峰，杀修双全 | 单价×2，不可复制不可遗传 |
| 🧠军师 | STRATEGY·纯信息调度 | 基类遍历分类，标记队友精准目标 | 不杀Bug，单价1x |
| ⚡闪电 | LIGHTNING·跑图支援 | 每轮重扫最密区，沦陷MVP | 不可复制，6轮必死 |

### 指挥官专武绑定系统

- **八种指挥官**（爆裂/哨卫/侦察/收集/攻坚/尖刀/军师/闪电）× **八种专属职业**，默认独占绑定
- 专武仅在专属职业+专属指挥官组合下生效，无物理增强——只改调度方式
- 开发者开关：开放模式下所有人可跨用，但非原配无专武增益
- 尖刀指挥官(精英渗透)=精英型天配，人数硬上限，个个以一当十

### 军师三层体系

- **军师指挥官**（决策层）→ **子军师/子进程**（协调层）→ **军师体/进程体**（贴身层）
- 纯信息调度：不杀Bug、不加属性。给坐标、标记精准目标、基类遍历分类
- 权限>0.5级（全指挥官第二），能调动其他指挥官但不新增执行代码
- 与 SceneEngine.derive() 同构——不是加强军队，是派最合适的兵去最合适的地方

### 闪电体系

- **闪电指挥官**→**闪电子进程**→**闪电体**：消耗不变，每轮动态重分配
- 跑图支援：扫描全场找Bug最密区→打完立刻换区→"支援贼快"
- 和军师对称：军师管初始分配（编译前），闪电管动态再分配（运行时）
- 绝境验证：15兵对700Bug→R3翻盘；500闪电+100爆裂+100攻坚对2500Bug→R1全灭

### 双维动态复制率

- **兵力缺口 × Bug压力**：gap越大复制率越高，Bug=0时降至3%最低维护
- Bug突然涌入→pressure乘数拉满→瞬间补兵；没Bug时不对空气挥拳
- 爆裂复制降5倍极限测试下仍能守住五区防线

### 自噬式吞并 & 水平基因转移

- 异化进程体：自噬（动态衰减）+吞并（死亡40%经验遗传）+分裂（原地复制）
- 水平基因转移：死后经验入公共池，任何变体都能捡——超级细菌质粒传递同构
- 沙盒清零：编译结束全员处决，顶旧pid干净重生。无清零→10轮标签污染不可逆发散

### 助攻追踪机制

- 区分三种状态：💤纯睡觉（不标记不杀）/ 🤝被抢人头（标记了但别人先杀）/ ⚔独狼（全自己杀）
- 收集型=被抢人头之王（0杀1612助攻），爆裂=抢不过（9杀372助攻），军师调度15次助攻全返给队友

### 增派预设模板

- deployArmy不再全补列兵→按ArmyPresetManager预设比例补人
- 模板驱动（闪电战60%闪电/持久战50%收集/防守型70%哨卫/均衡各20%）
- 不推导战场→开发者预设偏好→增派器按比例执行

### 设计哲学

> 「不是每个指挥官都适合持久战，有的就该一波推平。」
> 「军师不给数值，给时机——告诉爆裂这轮别炸、下轮再炸。」
> 「闪电不是加速站桩，是跑图支援——打完就跑，永远在最密区。」
> 「助攻机制回答：谁在干活、谁被抢、谁在睡——一目了然。」
