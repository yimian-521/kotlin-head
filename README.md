...
（八种进程体 = 从 v0.11.2 十一种子进程职业精选重定义，同一概念，聚焦生态位）
| 进程体 | 倾向 | 生态位 | 代价 |
|--------|------|--------|------|
...
| 👑精英 | ELITE·天生全能 | 开局即巅峰，杀修双全 | 单价×2，不可复制 |
| 🧠军师 | STRATEGY·纯信息 | 基类遍历分类，标记队友 | 不杀Bug，单价1x |
| ⚡闪电 | LIGHTNING·跑图 | 每轮重扫最密区，沦陷MVP | 不可复制，6轮必死 |

**子进程协同模式**（子进程=协调层，可派兵;进程体=执行层，杀Bug。五种协作方式）
- 均切（分片）：均切认领各干各的
- 流水线（接力）：多阶段接力，上游产出→下游复制走人
- 竞合（双跑）：关键路径双跑，指挥官仲裁选优
- 侦查（探路）：先派探路通了再批量投
- 收集（汇总）：不拆任务等结果统一汇总
...
| 均切 | 分片——均切认领各干各的 | 全部 |
| 流水线 | 接力——多阶段接力，上游产出下游复制 | 全部 |
| 竞合 | 双跑——关键路径双跑，指挥官仲裁选优 | 全部 |
| 侦查 | 探路——先派探路通了再批量投 | 全部 |
| 收集 | 汇总——不拆任务等结果统一汇总 | 全部 |
