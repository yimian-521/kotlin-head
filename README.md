...
**v1.0.5** — 25模块重构 + int标准变更：告别凑数，全真实规则（2026-07-12 免免 &amp; 望安）
> BugDB从5000条(89真+4911凑)→2937条全真实规则。int标准重定义：int+不再=5000条占位符，int++=2937全真实。25模块本质化重构（三分索引+去中心调度+反向索引）。178S十条满分(8/8)。
