...
### 测试方法论等同性证明（非官方认证）
> 以下测试遵循DO-178C §6.4 测试方法论，但由开发者自主实施，未经FAA/EASA认证审计。

| DO-178C 测试方法 | 实际操作 |
|----------------|---------|
| **等价类划分** (§6.4.2a) | 地狱v2：20种语法分类覆盖（try-catch/when/for/lambda/泛型/委托/注解等） |
| **边界值分析** (§6.4.2b) | 单token文件 / 空文件 / 仅package声明 / 恶意嵌套括号 |
| **错误推测** (§6.4.2c) | 地狱v3：12条已知Kotlin编译器bug + 三件套生态漏洞 |
| **语句覆盖** (§6.4.4.2a) | 10项容错机制对应测试矩阵（见§9）+ 綦桐30文件全层覆盖 |
| **判定覆盖** (§6.4.4.2b) | 每个容错点验证两种路径：正常解析路径 + 异常→跳过路径 |
| **回归测试** (§6.4.4.3) | v0.1.0→v0.11.0 綦桐通过率 1/32→30/30，零回归崩溃 |
